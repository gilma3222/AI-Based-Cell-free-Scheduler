...
if __name__ == "__main__":
    ...
    for drop in range(9):
        ...
        for run in range(2000):
            ...
            for i in range(nf):
                A_i = state[0][action[i][0]*2] - state[0][action[i][0]*2+1] + state[0][action[i][1]*2+1] - state[0][action[i][1]*2]
                if A_i<8:
                    if clk[i][1] < clk[i][0]:
                    ...
                    for kkk in sortedbuffer[2*nf:]:
                        ...
                        for j in toolate:
                            if kkk==j:
                                ...
                                break
                        if flag:
                            if state[0][kkk[0] * 2] - state[0][kkk[0] * 2 + 1] + state[0][action[i][1] * 2 + 1] - state[0][action[i][1] * 2] > 8:
                                action[i][0] = kkk[0]
                                toolate.append(kkk)
                                if state[0][action[i][0] * 2] - state[0][action[i][0] * 2 + 1] < 4:
                                    pAction[i][0] = -(state[0][action[i][0] * 2] - state[0][action[i][0] * 2 + 1] - 4)
                    ...
                    for kkk in sortedbuffer[2*nf:]:
                        ...
                        for j in toolate:
                            if kkk==j:
                                ...
                                break
                        if flag:
                            if state[0][kkk[0] * 2 + 1] - state[0][kkk[0] * 2] + state[0][action[i][0] * 2] - state[0][action[i][0] * 2 + 1] > 8:
                                action[i][1] = kkk[0]
                                toolate.append(kkk)
                                if state[0][action[i][0] * 2] - state[0][action[i][0] * 2 + 1] < 4:
                                    pAction[i][0] = -(state[0][action[i][0] * 2] - state[0][action[i][0] * 2 + 1] - 4)
