...
tstate = np.load('perfect_state_8x2_10k_set0.npy')
tbuff = np.load('perfect_buffer_8x2_10k_set0.npy')
tlab = np.load('perfect_labels_8x2_10k_set0.npy')
for set_index in range(1,50):
    tmpstate = np.load('perfect_state_8x2_10k_set{}.npy'.format(set_index))
    tmpbuff = np.load('perfect_buffer_8x2_10k_set{}.npy'.format(set_index))
    tmplab = np.load('perfect_labels_8x2_10k_set{}.npy'.format(set_index))
...
ttbuff = np.reshape(tbuff[0:runs], (runs,32))
ttstate = np.reshape(tstate[0:runs], (runs,560))
ttlab = np.reshape(tlab[0:runs], (runs,16))
ttlabX = ttlab.copy()
######YYYYYY
# inputl = [[ttstate[i][8*ttlab[i][c]+k] for c in range(16) for k in range(8)] for i in range(runs)]
inputl = [[ttstate[i][8*ttbuff[i][2*c]+k] for c in range(16) for k in range(8)] for i in range(runs)]
##########^^^^^^
outlist = []
outputd = ttlab.copy()
for k in range(runs):
...
def train(epoch):
    ...
    trainloss=0.
    trainaccurecy = 0
    suffle=np.random.permutation(np.int(runs/320)*32)
    inputd = torch.from_numpy(inputnp)
    outputd =  torch.from_numpy(outputnp)
    count=0
    for i in range(0, np.int(runs/320)*32, 32):
        count += 1
    ...
    trainloss /= count
    train_accurecy.append(trainaccurecy.cpu().numpy()/(count*32*16))
...
def test():
    ...
    count=0
    with torch.no_grad():
        for i in range(np.int(runs/320)*32, np.int(runs/32)*32, 32):
            count+=1
    ...
    print('\nTest set: Average loss: {:.4f}, Accuracy: {}%\n'.format(
        test_loss, 100*correct/(count*32*16)))
        # 100. * correct / len(test_loader.dataset)))

    # save the result - Cost and Accuracy
    correct_list.append(correct.cpu().numpy()/(count*32*16))
...
plt.ylabel('MSE loss')
plt.legend()
plt.savefig('/home/gilmam/Desktop/study/scheduler/data8x50/gil/Loss_fig_perfect_mse_8x2_wd0_5mt.png')
...
plt.ylabel('Accuracy')
plt.legend()
plt.savefig('/home/gilmam/Desktop/study/scheduler/data8x50/gil/Accuracy_fig_perfect_mse_8x2_wd0_5mt.png')
